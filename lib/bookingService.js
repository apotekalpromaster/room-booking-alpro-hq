...
export async function getAvailability(dateStr) {
  ...
  // Fallback Mock Store
  const db = readDb();
  const events = db.events.filter(e => e.date === dateStr && e.status !== 'CANCELLED');
...
export async function createBooking(data) {
  ...
  if (clients) {
    ...
    if (concurrentEvents.length > 1) {
      ...
      // Jika event kita bukan yang paling pertama masuk -> batalkan event ini (self-delete)
...
export async function getHistory(filters = {}) {
  ...
  if (clients && clients.spreadsheetId) {
    try {
      ...
      console.error('Error fetching Google Sheets history, fallback to mock:', err);
    }
  }
